...
| `src/probes/budget.ts`                                                                                                                                                | The per-probe ceiling the runner enforces, plus the deadline a multi-hop probe sizes its hops against. A probe that overruns has its verdict discarded and replaced with `unknown`, so budgets must not sum past it.                                                   |
| `src/probes/fetch-error.ts`                                                                                                                                           | Operator-readable `fetch` failures: distinguishes refused (service down) from silent (route black-holed) and digs undici's errno out of `cause`.                                                                                                                       |
