...
- **SignalK health probe is SSL-aware (observes runtime, doesn't read config).** signalk-server's HTTP port is `80` by default (the installer's privileged-ports default; `SIGNALK_URL`/`SIGNALK_HTTPS_URL` are templated into our Quadlet from `SK_HTTP_PORT`/`SK_HTTPS_PORT`). When the operator enables TLS (e.g. via signalk-ssl), signalk-server serves HTTPS on `sslport` (443) and the HTTP port **302-redirects** to it — it does not close. So `signalk-health.ts` fetches HTTP with `redirect: 'manual'`: a 2xx means SSL-off-and-healthy; a 3xx/`opaqueredirect` means SSL is on, and it follows to `SIGNALK_HTTPS_URL` with TLS verification relaxed (the leaf is self-signed by signalk-ssl's local CA — an internal host-gateway hop, not a browser). We deliberately do **not** mount `~/.signalk` to read `ssl`/`sslport`; observing 200-vs-302 self-heals when SSL is toggled. `src/probes/signalk-url.ts` is the single source of both URLs.
...
| `webapp/src/App.tsx`                                                                                                  | Hash-routed tab shell. Views: Health / Logs / Snapshots / Recovery / Installer.                                                                                                        |
