...
// Node 20+ enables autoSelectFamily by default with a 250ms per-address cap.
// On a dual-stack host (ghcr.io and the npm registry are Cloudflare-fronted
// and resolve A+AAAA) reached over a slow link (boat LTE/satellite), the
// first address family's connect can't complete inside 250ms, so the whole
...
// (|| handles NaN), and the floor of 250 (Node's own default) keeps a
// misconfigured "0"/"50" from re-introducing the very instant-fail this
